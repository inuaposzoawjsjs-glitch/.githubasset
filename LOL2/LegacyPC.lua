...
local diedConn = nil
local originalSlideMove = nil

function DFunctions.StartInfiniteSlide()
    local p = game.Players.LocalPlayer
    local c = p.Character or p.CharacterAdded:Wait()
    local h = c:WaitForChild("HumanoidRootPart", 8)
    ...
    local function overrideSlideFunction(moduleScript)
        local success, sm = pcall(require, moduleScript)
        if success and sm and sm.SlideMove and sm.SlideMove ~= originalSlideMove then
            originalSlideMove = sm.SlideMove
            sm.SlideMove = function(self, dt)
                if DConfiguration.InfiniteSlide.Enabled and hum and hum.Health > 0 then
                    if slideDirection == Vector3.zero and hum.MoveDirection.Magnitude > 0.1 then
                        slideDirection = h.CFrame.LookVector
                    end
                    if slideDirection ~= Vector3.zero then
                        self.d = (self.d or Vector3.zero) + (slideDirection * sf)
                    end
                end
                return originalSlideMove(self, dt)
            end
        end
    end

    local path = workspace.Game.Players:FindFirstChild(p.Name)
    if path then
        local m = path:FindFirstChild("Movement", true)
        if m and m:IsA("ModuleScript") then
            overrideSlideFunction(m)
        end
    end

    local mm = c:FindFirstChild("Movement", true)
    if mm and mm:IsA("ModuleScript") then
        overrideSlideFunction(mm)
    end

    if slideHeartbeat then slideHeartbeat:Disconnect() end
    slideHeartbeat = rs.Heartbeat:Connect(function()
        if not DConfiguration.InfiniteSlide.Enabled then 
            slideDirection = Vector3.zero
            return 
        end
        if not c or not h or not h.Parent or not hum or hum.Health <= 0 then
            slideDirection = Vector3.zero
            return
        end
        if slideDirection ~= Vector3.zero then
            if hum.MoveDirection.Magnitude > 0.1 and hum.MoveDirection.Unit:Dot(slideDirection.Unit) < 0.5 then
                slideDirection = Vector3.zero
                return
            end
            h.CFrame = CFrame.new(h.Position, h.Position + slideDirection)
        end
    end)
    ...
    charAddedConn = p.CharacterAdded:Connect(function(newChar)
        c = newChar
        h = newChar:WaitForChild("HumanoidRootPart")
        hum = newChar:WaitForChild("Humanoid")
        slideDirection = Vector3.zero
        
        task.wait(1)
        local newMm = c:FindFirstChild("Movement", true)
        if newMm and newMm:IsA("ModuleScript") then
            overrideSlideFunction(newMm)
        end
    end)
end

function DFunctions.StopInfiniteSlide()
    DConfiguration.InfiniteSlide.Enabled = false
    if slideHeartbeat then slideHeartbeat:Disconnect() slideHeartbeat = nil end
...
Tabs.Misc:AddToggle("InfiniteSlideEvade", {
    Title = "Infinite Slide", 
    Default = false,
    Callback = function(State)
        DConfiguration.InfiniteSlide.Enabled = State
        if State then
            DFunctions.StartInfiniteSlide()
        else
            DFunctions.StopInfiniteSlide()
        end
    end
})
