...
-- Premium 
 ...
 do
    ...
    Tabs.Premium:AddToggle("RingToggle", {
    ...
    Tabs.Premium:AddInput("DurationInput", {
 ...
 Tabs.Premium:AddSection("Crosshair")
 ...
 do
    ...
    local function updateLogoProperties()
        local lp = game:GetService("Players").LocalPlayer
        local pgui = lp:FindFirstChild("PlayerGui")
        local sg = pgui and pgui:FindFirstChild("GribabasCenter")
        if sg then
            local img = sg:FindFirstChild("RotatingLogo")
            if img then
                local targetSize = PremiumConfig.UseCustomSize and PremiumConfig.Size or PremiumConfig.DefaultSize
                local targetAsset = PremiumConfig.UseCustomAsset and PremiumConfig.AssetId or PremiumConfig.DefaultAsset
                
                img.Size = UDim2.new(0, targetSize, 0, targetSize)
                img.Image = "rbxassetid://" .. targetAsset:gsub("%D", "")
                
                if not PremiumConfig.SpinEnabled then
                    img.Rotation = 0
                end
            end
        end
    ...
    local function startCheat()
        ...
        if pgui:FindFirstChild("GribabasCenter") then return end

        local sg = Instance.new("ScreenGui")
        sg.Name = "GribabasCenter"
        sg.IgnoreGuiInset = true
        sg.ResetOnSpawn = false
        sg.Parent = pgui

        local img = Instance.new("ImageLabel", sg)
        img.Name = "RotatingLogo"
        ...
        task.spawn(function()
            while img and img.Parent and PremiumConfig.Enabled do
                if PremiumConfig.SpinEnabled then
                    img.Rotation = img.Rotation + PremiumConfig.RotationSpeed
                else
                    img.Rotation = 0
                end
                task.wait()
    ...
    Tabs.Premium:AddToggle("MainLogoToggle", {
        Title = "Show Logo",
        Default = false,
        Callback = function(Value)
            PremiumConfig.Enabled = Value
            if Value then startCheat() else updateLogoProperties() end
        end
    })

    Tabs.Premium:AddToggle("AssetToggle", {
        Title = "Use Custom ID",
        Default = false,
        Callback = function(Value)
            PremiumConfig.UseCustomAsset = Value
            updateLogoProperties()
        end
    })

    Tabs.Premium:AddInput("AssetInput", {
        Title = "Custom Asset ID",
        Default = PremiumConfig.AssetId,
        Numeric = true,
        Finished = true,
        Callback = function(Value)
            PremiumConfig.AssetId = Value
            if PremiumConfig.UseCustomAsset then updateLogoProperties() end
        end
    })

    Tabs.Premium:AddToggle("SpinToggle", {
        Title = "Enable Spin",
        Default = false,
        Callback = function(Value)
            PremiumConfig.SpinEnabled = Value
            updateLogoProperties()
        end
    })

    Tabs.Premium:AddSlider("SpinSlider", {
        Title = "Spin Speed",
        Default = 5,
        Min = 1,
        Max = 100,
        Rounding = 1,
        Callback = function(Value)
            PremiumConfig.RotationSpeed = Value
        end
    })

    Tabs.Premium:AddToggle("SizeToggle", {
        Title = "Enable Custom Size",
        Default = false,
        Callback = function(Value)
            PremiumConfig.UseCustomSize = Value
            updateLogoProperties()
        end
    })

    Tabs.Premium:AddSlider("SizeSlider", {
        Title = "Custom Size",
        Default = 30,
        Min = 10,
        Max = 300,
        Rounding = 0,
        Callback = function(Value)
            PremiumConfig.Size = Value
            if PremiumConfig.UseCustomSize then updateLogoProperties() end
        end
    })

    task.spawn(function()
        while true do
            if PremiumConfig.Enabled then
                local lp = game:GetService("Players").LocalPlayer
                local pgui = lp:FindFirstChild("PlayerGui")
                if pgui and not pgui:FindFirstChild("GribabasCenter") then
                    startCheat()
                end
            end
            task.wait(1)
...
local FakeSection = Tabs.Premium:AddSection("Pads")
...
do
    ...
    local tpDropdown = Tabs.Premium:AddDropdown("TeleporterList", {
...
do
    ...
    Tabs.Premium:AddToggle("SpeedPadToggle", {
    ...
    Tabs.Premium:AddInput("SpeedValueInput", {
    ...
    Tabs.Premium:AddInput("SpeedDurationInput", {
        Title = "Duration",
        Default = "2",
        Numeric = true,
    ...
    Tabs.Premium:AddToggle("JumpPadToggle", {
    ...
    Tabs.Premium:AddInput("JumpInput", {
        Title = "Jump Power",
        Default = "50",
        Numeric = true,
...
Tabs.Premium:AddToggle("BuildAnywhere", {
    ...
    Callback = function(State)
        ...
        if State then
            oldTestHit = buildModule.TestHit
            buildModule.TestHit = function(...)
                return false
            end

            oldTestCollision = buildModule.TestCollision
...
local FakeSection = Tabs.Premium:AddSection("AccessoryChanger")
...
do 
    Tabs.Premium:AddToggle("RemoveAccs", {Title = "Remove Accessories", Default = false})
...
local FakeSection = Tabs.Premium:AddSection("Grenade")
...
local FakeSection = Tabs.Premium:AddSection("Cola")
...
do 
    ...
    Tabs.Premium:AddToggle("ColaToggle", {
        Title = "Cola Button", 
        Default = false,
        Callback = function(State)
            if State then
    ...
    Tabs.Premium:AddInput("ColaSizeInput", {
        Title = "Cola Gui Size",
        Default = "1",
        Placeholder = "",
        Numeric = true,
...
Tabs.Premium:AddToggle("AutoDrinkToggle", {
...
Tabs.Premium:AddInput("DrinkDelayInput", {
...
Tabs.Premium:AddToggle("InfColaToggle", {
    Title = "Infinite Cola",
...
Tabs.Premium:AddInput("ColaSpeedInput", {
    Title = "Speed Multiplier",
    Default = "1.4",
    NumericOnly = true,
    Callback = function(Value)
...
Tabs.Premium:AddInput("ColaDurationInput", {
